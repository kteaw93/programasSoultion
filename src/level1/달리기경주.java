...
public class 달리기경주 {
	...
	private static String[] solution(String[] players, String[] callings) {
		HashMap<String, Integer> playersHash = new HashMap();
		int k = 0;
		for (int i = 0; i < players.length; i++) {
			playersHash.put(players[i] , i);
		}
		for (String call : callings) {
			int playerRank = playersHash.get(call);//호출된사람 자리
			String changePlayer = players[playerRank];//호출된사람 이름
			String changeRankName = players[playerRank -1];//호출된사람 앞자리 이름
			int changeRankNum = playersHash.get(changeRankName);//호출된사람 앞자리 이름
			
			playersHash.put(changeRankName , playerRank);
			playersHash.put(changePlayer , changeRankNum);
			players[playerRank] = changeRankName;
			players[playerRank-1] = changePlayer;
